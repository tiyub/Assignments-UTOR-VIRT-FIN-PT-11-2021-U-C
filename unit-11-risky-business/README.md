...
Jasen Mackie - Instructor So, there is some confusion from the assignment about why the accuracy score is similar for models making predictions given the same test data, trained on data that is (a) randomly oversampled, and (b) oversampled with SMOTE.These are my observations:

    1. The training data does not have very much variance...as if the data was synthetically generated from a very tight distribution.
    2. As a result, the train data generated from ROS and SMOTE is very similar, but not identical. This can be confirmed by summarizing each column (ie. feature) from both train datasets.
    3. So if the train data is different, then the models trained on the data should be different...so how is it that we are getting identical accuracy scores? Well, the models are different as are the probability estimates (ie. the prediction between 0 and 1). You can observe this quite simply by using code similar to above screenshot. However, the probability estimates are so close and most importantly the accuracy scores are identical because when each observations between both estimates is compared to the threshold, they predict the same class.
